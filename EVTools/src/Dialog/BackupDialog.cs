...
namespace Swsk33.EVTools.Dialog
{
	public partial class BackupDialog : Form
	{
		...
		/// 存放编码列表
		/// </summary>
		private static readonly Dictionary<string, Encoding> ENCODING_PAGE = new Dictionary<string, Encoding>();
		...
		static BackupDialog()
		{
			ENCODING_PAGE.Add("GBK", Encoding.GetEncoding("GBK"));
			ENCODING_PAGE.Add("UTF-8", new UTF8Encoding(false));
			ENCODING_PAGE.Add("UTF-8 with BOM", new UTF8Encoding(true));
			ENCODING_PAGE.Add("UTF-16", Encoding.GetEncoding("UTF-16"));
			ENCODING_PAGE.Add("ASCII", Encoding.GetEncoding("ASCII"));
		}

		public BackupDialog()
		{
			InitializeComponent();
			foreach (string key in ENCODING_PAGE.Keys)
			{
		...
		private void pathBackupButton_Click(object sender, EventArgs e)
		{
			...
			if (dialog.ShowDialog() == DialogResult.OK)
			{
				...
				new Thread(() =>
				{
					...
					File.WriteAllText(dialog.FileName, content, ENCODING_PAGE[encodingBox.SelectedItem.ToString()]);
		...
		private void allBackupButton_Click(object sender, EventArgs e)
		{
			...
			if (dialog.ShowDialog() == DialogResult.OK)
			{
				...
				new Thread(() =>
				{
					...
					File.WriteAllLines(dialog.FileName, contents.ToArray(), ENCODING_PAGE[encodingBox.SelectedItem.ToString()]);
					MessageBox.Show("备份脚本已导出至：" + dialog.FileName, "完成", MessageBoxButtons.OK, MessageBoxIcon.Information);
					operateButtons(true);
				}).Start();
			}
